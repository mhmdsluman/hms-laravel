...
// common department suggestions and solid color mapping
const deptOptions = [
  'Surgery',
  'Laboratory',
  'Radiology',
...
const submit = () => {
  ...
  form.post(route('services.store'), {
    ...
    onError() {
      // server-side errors will be available on form.errors
      submitting.value = false;
    },
  });
};

// helper to pick a department from suggestions
const chooseDept = (d) => {
  form.department = d;
};
</script>
...
<template>
  ...
  <AuthenticatedLayout>
    <template #header>
      <h2 class="font-semibold text-lg text-gray-900 truncate">Add New Service</h2>
    </template>
      ...
      <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow-lg rounded-2xl overflow-hidden">
          <div class="p-6 border-b flex items-start justify-between gap-4">
            <div>
              <h3 class="text-lg font-semibold text-gray-800">Create Service</h3>
              <p class="text-sm text-gray-500 mt-1">Add a catalogue service — pick a department, set price and code.</p>
            </div>

            <!-- Live preview card -->
            <div class="flex-shrink-0">
              <div class="w-48 rounded-lg p-3 border flex flex-col gap-2">
          ...
          <form @submit.prevent="submit" class="p-6 space-y-6">
            ...
            <!-- Department (datalist + quick picks) -->
            <div>
            ...
            <div>
              ...
              <textarea
                ...
                class="mt-2 block w-full rounded-lg border-gray-200 shadow-sm focus:ring-2 focus:ring-indigo-200"
                placeholder="Short description or instructions shown to clinicians..."
              ></textarea>
            ...
            <div class="flex items-center justify-end gap-3">
              ...
              >
                ...
                <span>{{ form.processing || submitting ? 'Saving...' : 'Save Service' }}</span>
              </button>
            </div>
          </form>
        </div>
...
/* make suggested dept pills look clickable and consistent */
[role="button"], .cursor-pointer { cursor: pointer; }

/* ensure datalist looks okay on some browsers (minor) */
