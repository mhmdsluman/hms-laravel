...
function goToInventory() {
  const url = safeRoute('inventory.index');
  if (url) window.location.href = url;
  else alert('Inventory route not registered.');
}
    ...
    <template #header>
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 w-full">
        ...
        <div class="flex items-center gap-2">
          <button @click="goToInventory" class="px-4 py-2 bg-gray-800 text-white rounded-md text-sm hover:bg-gray-900">Manage Inventory</button>
                  ...
                  <tr v-for="item in paged" :key="item.id" class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-right text-sm font-medium">
                      <div class="inline-flex items-center gap-2">
                        ...
                        <!-- Dispense link/button - uses safeRoute to avoid Ziggy throw -->
                        <Link
                        ...
                        <!-- fallback: client-side confirm & post -->
