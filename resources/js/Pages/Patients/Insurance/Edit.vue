...
const form = useForm({
    insurance_provider_id: props.policy.insurance_provider_id,
    policy_number: props.policy.policy_number,
    group_number: props.policy.group_number,
    start_date: props.policy.start_date,
    end_date: props.policy.end_date,
});

const submit = () => {
    form.put(route('insurance-policies.update', props.policy.id));
};
</script>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Edit Insurance Policy for {{ policy.patient.first_name }} {{ policy.patient.last_name }}
            </h2>
        </template>
        <div class="py-12">
                    ...
                    <div class="p-6 bg-white border-b border-gray-200">
                        <form @submit.prevent="submit">
                            ...
                            <div class="flex items-center justify-end mt-6">
                                <Link :href="route('patients.show', policy.patient_id)" class="text-sm text-gray-600 hover:underline">Cancel</Link>
                                <button type="submit" :disabled="form.processing" class="ml-4 px-4 py-2 bg-green-600 text-white rounded-md">Update Policy</button>
                            </div>
                        </form>
                    </div>
                </div>
